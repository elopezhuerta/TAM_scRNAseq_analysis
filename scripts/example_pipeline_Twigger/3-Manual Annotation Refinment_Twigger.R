...
annotated_seurat <- readRDS(file = "results/seurat_objects/TwiggerDataset/SnR.Annotated_Twigger.rds")
...
# T CD8
cd8_cells <- WhichCells(lymphoid_cells, expression = CD8A >= 1)
annotated_seurat$Annotation <- ifelse(
  colnames(annotated_seurat) %in% cd8_cells & 
    annotated_seurat$seurat_clusters == "1" & 
...
# Regulatory T cells
treg_cells <- WhichCells(lymphoid_cells, expression = CTLA4 >= 2)
annotated_seurat$Annotation <- ifelse(
...
# NK cells
nk_cells <- WhichCells(lymphoid_cells, expression = KLRD1 >= 2)
annotated_seurat$Annotation <- ifelse(
...
# Reinforce correct B cell label for cluster 10
annotated_seurat$Annotation <- ifelse(
  annotated_seurat$seurat_clusters == "10" & annotated_seurat$Annotation == "SnR.B_cell",
  "B cell",
  annotated_seurat$Annotation
)
...
myeloid_subset <- subset(
  ...
  subset = Annotation %in% c("SnR.Macrophage", "SnR.Monocyte", "SnR.DC", "SnR.Neutrophils")&
    seurat_clusters %in% c(likely_myeloid_clusters, 4, 17)
)
...
annotated_seurat$Annotation <- ifelse(
  annotated_seurat$Annotation %in% c("SnR.Macrophage", "SnR.Monocyte", "SnR.DC") &
    annotated_seurat$seurat_clusters %in% c(4, 12,15,17,10),
  "Tissue macrophages",
  annotated_seurat$Annotation
)
...
annotated_seurat$Annotation <- ifelse(
  colnames(annotated_seurat) %in% cDC1_cells & annotated_seurat$seurat_clusters == "12",
  "cDC1",
  annotated_seurat$Annotation
)
...
annotated_seurat$Annotation <- ifelse(
  colnames(annotated_seurat) %in% cDC2_cells & annotated_seurat$seurat_clusters == "12",
  "cDC2",
  annotated_seurat$Annotation
)

# Mature DC
mature_dc_cells <- WhichCells(myeloid_subset, expression = LAMP3 >= 2.5 | BIRC3 >= 2.5)
annotated_seurat$Annotation <- ifelse(
  colnames(annotated_seurat) %in% mature_dc_cells & annotated_seurat$seurat_clusters %in% c(4, 12),
  "Mature DC",
  annotated_seurat$Annotation
)
...
gridExtra::grid.arrange(grobs = violins_stromal, ncol = 3)
...
# Fibroblasts (unify related subtypes in cluster 13)
annotated_seurat$Annotation <- ifelse(
  annotated_seurat$seurat_clusters == "13" &
    annotated_seurat$Annotation %in% c("SnR.Fibroblasts", "SnR.Smooth_muscle_cells", "SnR.Tissue_stem_cells", "SnR.Chondrocytes"),
  "Fibroblasts",
  annotated_seurat$Annotation
)

# Pericytes (clusters 7 and 17)
annotated_seurat$Annotation <- ifelse(
  annotated_seurat$seurat_clusters %in% c(7, 17) &
    annotated_seurat$Annotation %in% c("SnR.Fibroblasts", "SnR.Smooth_muscle_cells", "SnR.Tissue_stem_cells", "SnR.Chondrocytes"),
  "Pericytes",
  annotated_seurat$Annotation
)
...
# ----------------------------------------------------------------------------
#  No clear clusters enriched for cycling markers (MKI67, CDK1)
# ============================================================================
...
combined_umap <- combined_umap+labs(title = "Twigger dataset: Normal mammary tissue")

ggsave(filename = file.path(fig_dir, "Twigger_UMAP_Annotation.pdf"),
...
final_dotplot <- DotPlot(
  ...
  )+labs(title = "Twigger dataset: Normal mammary tissue")

ggsave(filename = file.path(fig_dir, "Twigger_DotPlot_Annotation.pdf"),
