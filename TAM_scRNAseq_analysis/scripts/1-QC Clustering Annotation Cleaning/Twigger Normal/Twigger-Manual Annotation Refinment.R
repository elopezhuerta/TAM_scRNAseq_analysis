...
FeaturePlot(Lymph, features = c("IGHG4","IGHG1","IGHA1"),slot = "data",cols = reds)
...
Annotated$Annotation<-ifelse(Annotated$seurat_clusters%in%c("1")&
                             ...
                             as.character(Annotated$Annotation))
#Defining Tregs
treg<-WhichCells(Lymph, expression = CTLA4 >= 2)
Annotated$Annotation<-ifelse(colnames(Annotated)%in%treg,
                             "T regs",
                             as.character(Annotated$Annotation))

#Defining NK
NK<-WhichCells(Lymph, expression = KLRD1 >= 2)
Annotated$Annotation<-ifelse(colnames(Annotated)%in%NK,
                             "NK cell",
                             as.character(Annotated$Annotation))
...
Myeloid<-subset(Annotated,
                ...
                subset=Annotation%in%c("SnR.Macrophage","SnR.Monocyte","SnR.DC",
                  ...
                  seurat_clusters%in%c(Myel.Cl,4,17,9))
...
# CONCLUSION ACCORDING TO MARKER EXPRESSION PATTERN
# Tissue macrophages 
# CD14 monocytes are in cluster 15 but also epithelial (Duplets)
# DC pops are in cluster 12
# No neutrophils nor mast cells
...
Annotated$Annotation<-ifelse(Annotated$Annotation%in%c("SnR.Macrophage","SnR.Monocyte",
                                                       "SnR.DC")&
                             Annotated$seurat_clusters%in%c(4,17,9,12),
                             "Tissue macrophages",
                             as.character(Annotated$Annotation))
...
Annotated$Annotation<-ifelse(colnames(Annotated)%in%cDC1&
                               Annotated$seurat_clusters%in%c(12),
                             "cDC1",
                             as.character(Annotated$Annotation))
...
Annotated$Annotation<-ifelse(colnames(Annotated)%in%cDC2&
                             ...
                             as.character(Annotated$Annotation))
#Mature DC
MDC<-WhichCells(Myeloid, expression = LAMP3>= 2.5|BIRC3>= 2.5)
Annotated$Annotation<-ifelse(colnames(Annotated)%in%MDC&
                               Annotated$seurat_clusters%in%c(4,12),
                             "Mature DC",
                             as.character(Annotated$Annotation))
# ============================================================================
...
Annotated$Annotation<-ifelse(Annotated$seurat_clusters%in%c(13)&
...
Annotated$Annotation<-ifelse(
  #Annotated$seurat_clusters%in%c(0,2,5,10,14)&
                               Annotated$Annotation%in%c("SnR.Epithelial_cells"),
                             "Epithelial cells",
                             as.character(Annotated$Annotation))
#Defining Endothelial cells
Annotated$Annotation<-ifelse(
  #Annotated$seurat_clusters%in%c(3,6,8,11)&
...
#saveRDS(Annotated, file = "D:/R_ScriptsPaper/Objects/Twigger_HNormal.rds")
#
# ============================================================================
